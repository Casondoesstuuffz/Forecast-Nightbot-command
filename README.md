...
!forecast for Dallas TX on Tuesday at 7am
...
- **Flexible day input** — full weekday names (`thursday`) or abbreviations (`thu`, `thurs`), plus `today` and `tomorrow`
- **Date support** — `MM/DD/YYYY` or `M-D-YY` formats, e.g. `7/4/26`
- **Time-of-day support** — ask for a specific hour, e.g. `!forecast Sherman today 5:00pm` or `!forecast Dallas 7am`, pulled from hourly forecast data
- **Multiple locations and days in one query** — `!forecast Sherman TX and Greenville TX for Sunday and Monday` returns both cities across both days in a single response (capped at 2 locations × 2 days to stay within Nightbot's response length limit)
- **Natural language phrasing** — filler words like `for`, `on`, `at`, `the`, `in` are stripped automatically, so `!forecast for Dallas TX on Tuesday at 7am` works the same as the terse version
- **3-day outlook fallback** — if no day/date/time is given, returns the next 3 days
...
- **Built-in timeouts** — every external fetch is capped so the command never hangs Nightbot
...
For spam control, add a cooldown when creating or editing the command:

```
!commands edit !forecast -cd=120 $(urlfetch https://YOUR-DEPLOYMENT-URL?q=$(querystring))
```

`-cd` sets a global cooldown in seconds (blocks the command for everyone during that window, not per-user). Nightbot requires the full command to be re-specified on every edit — flags alone won't work.
...
- Multi-location/multi-day queries are capped at 2 locations × 2 days to stay within Nightbot's response length limit
